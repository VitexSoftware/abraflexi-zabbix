...
class Feeder
{
    ...
    private function colorizeJson(string $json): string
    {
        ...
        $json = preg_replace_callback('/:\s*(-?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?)/', function($matches) {
            return ': ' . self::setColor($matches[1], 'yellow'); // Numbers in yellow
        }, $json);
        
        $json = preg_replace_callback('/:\s*(true|false)/', function($matches) {
            return ': ' . self::setColor($matches[1], 'red'); // Booleans in red
        }, $json);
        
        $json = preg_replace_callback('/:\s*(null)/', function($matches) {
